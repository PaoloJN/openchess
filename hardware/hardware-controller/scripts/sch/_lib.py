"""_lib.py — shared utilities for the controller-board schematic pipeline.

The pipeline writes openchess-controller.kicad_sch as concatenated chunks.
...
- Component refs are firmware contract — see hardware-controller/scripts/README.md.
...
BOARD_DIR = SCRIPTS_DIR.parent                       # hardware-controller/
...
OUT_PATH = BOARD_DIR / "openchess-controller.kicad_sch"
...
PFX_USB        = "0b50" # USB-C receptacle + CC pulldowns (chunk 06)
PFX_PWR_RAIL   = "9007" # bulk caps on +5V_LED + +3V3 (chunk 06)
PFX_LDO        = "1d00" # +3V3 LDO (chunk 06)
PFX_PWR_LABEL  = "1ab6" # net labels emitted by chunk 06 (distinct from PFX_LABEL)
...
PIN_COORDS: dict[str, dict[str, tuple[float, float]]] = {
    ...
    # USB-C 16P receptacle. Many pins are stacked at the same sym (x, y) so
    # the "visible" pin in PIN_COORDS is sufficient — the hidden duplicates
    # (A9 VBUS, A12/B1/B12 GND, B4/B9 VBUS) auto-connect at the same point.
    "Connector:USB_C_Receptacle_USB2.0_16P": {
        ...
        "A7":  (15.24,   +2.54),  # D-
        "A8":  (15.24,  -12.7),   # SBU1
        "A9":  (15.24,  +15.24),  # VBUS (hidden, stacked)
        "A12": ( 0.0,   -22.86),  # GND (hidden, stacked)
        "B1":  ( 0.0,   -22.86),  # GND (hidden, stacked)
        "B4":  (15.24,  +15.24),  # VBUS (hidden, stacked)
        "B5":  (15.24,   +7.62),  # CC2
        "B6":  (15.24,   -5.08),  # D+
        "B7":  (15.24,    0.0),   # D-
        "B8":  (15.24,  -15.24),  # SBU2
        "B9":  (15.24,  +15.24),  # VBUS (hidden, stacked)
        "B12": ( 0.0,   -22.86),  # GND (hidden, stacked)
        "SH":  (-7.62,  -22.86),  # SHIELD
    },
    # AP2112K-3.3 extends AP2204K-1.5; pins live on the base symbol.
    "Regulator_Linear:AP2112K-3.3": {
        "1": (-7.62, +2.54),   # VIN
        "2": ( 0.0,  -7.62),   # GND
        "3": (-7.62,  0.0),    # EN (active-high)
        "4": (+5.08,  0.0),    # NC
        "5": (+7.62, +2.54),   # VOUT
    },
...
def load_symbol_def(lib_id: str) -> str:
    ...
    # If this symbol uses (extends "X"), rewrite the bare base name to the
    # fully-qualified "Library:X" form so KiCad resolves it inside the
    # schematic's lib_symbols block. Only rewrite bare names (no colon).
    extends_marker = '(extends "'
    ext_start = block.find(extends_marker)
    if ext_start != -1:
        name_start = ext_start + len(extends_marker)
        name_end = block.find('"', name_start)
        base_name = block[name_start:name_end]
        if ":" not in base_name:
            block = block[:name_start] + f"{lib_name}:{base_name}" + block[name_end:]
