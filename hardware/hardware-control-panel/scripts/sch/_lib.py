"""_lib.py — shared utilities for the control-panel schematic pipeline.

The pipeline writes openchess-control-panel.kicad_sch as concatenated chunks.
...
- Component refs are firmware contract — see hardware-control-panel/scripts/README.md.
...
BOARD_DIR = SCRIPTS_DIR.parent                       # hardware-control-panel/
...
OUT_PATH = BOARD_DIR / "openchess-control-panel.kicad_sch"
...
PIN_COORDS: dict[str, dict[str, tuple[float, float]]] = {
    ...
    "Connector_Generic:Conn_01x10": {
        **{str(p): (-5.08, 10.16 - (p - 1) * 2.54) for p in range(1, 11)},
    },
    "Device:LED": {
        "1": (-3.81, 0.0),
        "2": (+3.81, 0.0),
...
def emit_symbol(o: list[str], lib_id: str,
    ...
    for pin_num, pin_uuid in pin_uuids.items():
        ...
        o.append("\t\t)")
    o.append("\t\t(instances")
    o.append('\t\t\t(project "openchess-control-panel"')
