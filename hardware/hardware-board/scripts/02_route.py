...
PREREQUISITE — openchess-board.kicad_pcb must already have component
...
try:
    ...
    # ships with KiCad so plain `python3 02_route.py` Just Works.
    _KICAD_PYTHONS = [
...
# scripts/ dir.  This script is the 8×8 (full) version; the 3×3 lives at
# ──────────────────────────────────────────────────────────────────────────────
MATRIX_COLS = 8
MATRIX_ROWS = 8
...
# Hall row geometry (parametric on SQUARE_SIZE + BOARD_MARGIN).
# 8×8 board: square 31.75 mm (= 1.25 in), margin 14 mm. Total 282×282 mm.
# First Hall row at Y = 79.875, subsequent rows at +31.75 mm.
# KEEP IN SYNC with 01_chess_grid.py SQUARE_SIZE and BOARD_MARGIN constants.
# (Updated 2026-06-10 to match the folding board's 10 in = 254 mm play area.)
...
def route_5v_trunks(board: "pcbnew.BOARD") -> int:
    ...
    for row in range(LED_ROWS):
        ...
        # === Entry: bulk pin 1 → first LED VCC ===
        # 01_chess_grid.py places each row bulk directly above its row's
        # first LED so bulk pin 1 X == LED VDD pad X. So entry collapses
        # to a single vertical F.Cu segment — no 45° entry needed.
        first_vcc = leds_caps[0][0]
        add_track(board, bulk, first_vcc, nc, LAYER_5V_TRUNK, POWER_WIDTH_MM)
        count += 1
